  ...
  async getHistoricalData(request: MarketDataRequest): Promise<MarketDataResponse> {
    try {
      ...
      return {
        ...
        currentPrice: result.meta?.regularMarketPrice ?? (data.length > 0 ? data[data.length - 1].close : undefined),
        previousClose: data.length > 1 ? data[data.length - 2].close : result.meta?.chartPreviousClose,
